...
for frame in range(start_frame_no, end_frame_no):
	...
	for val in curr_frame_unique_foreground_values: # Iterates over all foreground regions
		...
		temp_Y = Y
		btemporal = True
		for temporal_val in range(0,3): # Evaluating temporal consistency of a particular foreground block
			no_of_projections = {}
			...
			for i in range(len(temp_X)): # Iterates over all 4x4 blocks in a particular foreground region
				...
				projected_x = (temp_X[i]*4 + vert_disp/4)/4 # row + vertical disp
				projected_y = (temp_Y[i]*4 + horz_disp/4)/4 # col + horizontal disp

				if projected_x>=0 and projected_x<height and projected_y>=0 and projected_y<width: # Corner Cases
		...
		if not btemporal:
			...
			for fore_temporal_val in range(0,3): # Evaluating temporal consistency of a particular foreground block
				no_of_projections = {}
				...
				for i in range(len(fore_temp_X)): # Iterates over all 4x4 blocks in a particular foreground region
					horz_disp = int(literal_eval\
								(spatial_mv_data[fore_temp_curr_frame, fore_temp_X[i], fore_temp_Y[i]])[-2])
					vert_disp = int(literal_eval\
								(spatial_mv_data[fore_temp_curr_frame, fore_temp_X[i], fore_temp_Y[i]])[-1])

					projected_x = (fore_temp_X[i]*4 + int(vert_disp)/4)/4 # row + vertical disp
					projected_y = (fore_temp_Y[i]*4 + int(horz_disp)/4)/4 # col + horizontal disp
					
					if projected_x>=0 and projected_x<height and projected_y>=0 and projected_y<width: # Corner Cases			
						if foreground_arr[fore_temp_curr_frame+1, projected_x, projected_y]!=0:
...
np.save('./../'+video_type+'/'+video_name+'/output/'+'pic_object_track.npy',copy_foreground_arr)